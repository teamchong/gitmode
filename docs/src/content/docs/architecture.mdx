...
Git objects are stored at `{owner}/{repo}/objects/{sha[0:2]}/{sha[2:]}` (matching git's loose object layout) as zlib-compressed data with a type prefix byte:
...
3. **Packfile unpack — Phase 1 (CPU-only):** Objects prepared in memory (SHA-1 hash + zlib compress)
4. **Packfile unpack — Phase 2 (I/O):** Batch write to R2 with 50 concurrent PUTs
5. Refs updated in DO SQLite
6. Commits indexed in DO SQLite (author, message, timestamp)
7. **Worktree materialization:**
   ...
   - **Full mode** (first push): writes all files
8. File sizes cached in DO SQLite for stats endpoint
