...
import { parseCommitFromRaw, type CommitInfo } from "./commit-parse";

// Minimal Env shape consumed by PackWorkerDO. Callers' Env can extend this.
export interface PackWorkerEnv {
  OBJECTS: R2Bucket;
}
...
export class PackWorkerDO extends DurableObject<PackWorkerEnv> {
  ...
  // === parse-commits: read commit objects from R2, return parsed metadata ===

  private async handleParseCommits(request: Request): Promise<Response> {
    const body = await request.json() as CommitParseRequest;
    const { repoPath, commits } = body;
    if (!repoPath || typeof repoPath !== "string") {
      return new Response("Missing repoPath\n", { status: 400 });
    }
    ...
    if (commits.length > MAX_OBJECTS_PER_BATCH) {
      return new Response("Too many commits\n", { status: 400 });
    }

    const repoPrefix = repoPath + "/";
    for (const c of commits) {
      if (c.chunkKey && !c.chunkKey.startsWith(repoPrefix)) {
        return new Response("Invalid key scope\n", { status: 400 });
      }
      if (c.looseKey && !c.looseKey.startsWith(repoPrefix)) {
        return new Response("Invalid key scope\n", { status: 400 });
      }
    }
    ...
    const allChunkKeys = new Set<string>();
    for (const c of commits) {
      if (c.chunkKey) allChunkKeys.add(c.chunkKey);
    }
    const chunkData = await this.fetchChunks(r2, [...allChunkKeys]);
    ...
    for (const commit of commits) {
      const raw = await this.readRawObject(wasm, r2, chunkData, commit);
      if (!raw) {
        errors.push({ sha: commit.sha, error: "object not found" });
        continue;
      }
      const info = parseCommitFromRaw(commit.sha, raw);
      if (!info) {
        errors.push({ sha: commit.sha, error: "not a commit object" });
        continue;
      }
      results.push(info);
    }
